...
class XmlGeneratorService
{
    public function generateBatchXml(array $rpsList, string $loteId = '1'): string
    {
        ...
        $cnpj = $dom->createElement('Cnpj', '00000000000000');
        $cpfCnpjPrestador->appendChild($cnpj);
        $loteRps->appendChild($cpfCnpjPrestador);

        $inscricao = $dom->createElement('InscricaoMunicipal', '000000');
        ...
        foreach ($rpsList as $rpsData) {
            ...
            $rpsElement->appendChild($dom->createElement('Status', '1'));
            ...
            $servico->appendChild($dom->createElement('IssRetido', $rpsData->servico->issRetido ?? 2));
     ...
     * ABRASF v2.04 CompNfse structure - VERSÃO CORRIGIDA
     */
    public function generateDominioXml(array $rpsList): string
    {
        ...
        foreach ($rpsList as $index => $rpsData) {
            ...
            // Ordem correta: Numero, CodigoVerificacao, DataEmissao
            // NÃO incluir Competencia aqui - XSD não espera
            $infNfse->appendChild($dom->createElement('Numero', $rpsData->numero));
            $infNfse->appendChild($dom->createElement('CodigoVerificacao', strtoupper(substr(md5($rpsData->numero), 0, 9))));
            $infNfse->appendChild($dom->createElement('DataEmissao', $rpsData->dataEmissao . 'T00:00:00'));
            ...
            $issRetido = $rpsData->servico->issRetido ?? 2;
            if ($issRetido == 0)
                $issRetido = 2;
            $servico->appendChild($dom->createElement('IssRetido', $issRetido));

            $servico->appendChild($dom->createElement('ItemListaServico', $rpsData->servico->itemListaServico));

            if (!empty($rpsData->servico->codigoCnae)) {
                $servico->appendChild($dom->createElement('CodigoCnae', $rpsData->servico->codigoCnae));
            }

            if (!empty($rpsData->servico->codigoTributacaoMunicipio)) {
                $servico->appendChild($dom->createElement('CodigoTributacaoMunicipio', $rpsData->servico->codigoTributacaoMunicipio));
            }
            ...
            // Prestador - ORDEM CORRETA: RazaoSocial PRIMEIRO
            $prestador = $dom->createElement('PrestadorServico');
            $infNfse->appendChild($prestador);

            // RazaoSocial VEM ANTES de CpfCnpj
            $prestador->appendChild($dom->createElement('RazaoSocial', 'PRESTADOR DE SERVICOS LTDA'));

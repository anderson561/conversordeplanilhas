...
class PdfParserService
{
    public function parse(string $filePath): array
    {
        ...
        $pattern1 = '/(\d{2}[\/\.]\d{2}[\/\.](?:\d{4}|\d{2}))\s+([\d\.,]+(?:R\$)?)\s+(.+?)\s+(\d{2,3}\.\d{3}\.\d{3}[\/\-]\d{2,4}-?\d{0,2})/u';

        // Pattern 2: Name | CNPJ | Value | Date (User Image)
        // Sample Name 12.345.678/0001-90 2.160,00 01/07/2025
        $pattern2 = '/(.+?)\s+(\d{2,3}\.\d{3}\.\d{3}[\/\-]\d{2,4}-?\d{0,2})\s+([\d\.,]+(?:R\$)?)\s+(\d{2}[\/\.]\d{2}[\/\.](?:\d{4}|\d{2}))/u';
        ...
        $pattern4 = '/(?:BANCO\s+\w+|[\w\s\.-]+?)\s+(\d{2}[\/\.]\d{2}[\/\.](?:\d{4}|\d{2}))\s+([\d\.,]+)\s*(.+?)\s+(\d{2,3}\.\d{3}\.\d{3}[\/\-]\d{2,4}-?\d{0,2})/ui';
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            // CPF: XXX.XXX.XXX-XX
            if (preg_match('/(\d{2}[\/\.]\d{2}[\/\.](?:\d{4}|\d{2}))\s+([\d\.,]+)(.+?)(\d{2,3}\.\d{3}\.\d{3}[\/\-]\d{2,4}-?\d{0,2})/u', $line, $matches)) {
